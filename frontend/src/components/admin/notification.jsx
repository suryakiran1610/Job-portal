...
function Notification(props) {
        ...
        const params = {
          ...
          MakeApiRequest(
            ...
            .then((response) => {
              ...
              setNotifications((prevNotifications) => ({
                ...prevNotifications,
                notification: prevNotifications.notification.map(
                    (notification) =>
  ...
  return (
    <div>
        <div  className="2xl:w-4/12  h-screen overflow-y-auto md:p-8 p-4 ">
                    ...
                    {notifications.notification.length > 0 ? notifications.notification.map((notification, index) => (
                        <div>
                                <div onClick={()=>{readed(notification.id)}} key={index} className={`w-full p-3 mt-4 rounded shadow hover:shadow-lg transition-shadow duration-300 ease-in-out flex flex-shrink-0 ${notification.isread ? 'bg-white' : 'bg-blue-200'}`}>
                                    ...
                                    <div className="pl-3 w-auto md:w-2/5 flex-col">
                                        <div className="flex items-center justify-between">
                                            <p tabIndex="0" className="focus:outline-none text-sm leading-none"><span className="text-indigo-700">{notification.companyname}</span> {notification.message} on {notification.registereddate}</p>
                                        </div>  
                                    </div> 

                                    {companyData.company.find(company => company.id === notification.companyid && company.is_active === false) ? (
                                        <div  className="flex cursor-pointer">
                                                    <div className="flex flex-col items-center mr-4 ml-4 ">
                                                        <div  className="flex items-center justify-center w-7 h-7 border-2 hover:border-4 border-green-500 rounded-full">
                                                            <svg onClick={()=>{approvecompany(notification.companyid)}} className="w-7 h-7 text-green-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    <div className="flex flex-col items-center">
                                                        <div className="flex items-center justify-center w-7 h-7 border-2 hover:border-4 border-red-500 rounded-full">
                                                            <svg onClick={()=>{handledeletemodal(notification.companyid)}} className="w-7 h-7 text-red-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
